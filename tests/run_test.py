...
class TestDeepInsight(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rand_input = np.sin(np.random.rand(int(self.input_length), self.input_channels))
        self.rand_input_timesteps = np.arange(0, self.input_length)
        self.rand_output = np.random.rand(self.input_length // self.input_output_ratio)
        self.rand_timesteps = np.arange(0, self.input_length, self.input_output_ratio)
    ...
    def test01_input_output(self):
        ...
        deepinsight.preprocess.preprocess_input(self.fp_deepinsight, self.rand_input, sampling_rate=self.sampling_rate)
    ...
    def test03_model_training(self):
        ...
        deepinsight.train.run_from_path(self.fp_deepinsight, loss_functions, loss_weights, user_opts)

    def test04_model_performance(self):
        # Get loss and shuffled loss for influence plot, both is also stored back to HDF5 file
        losses, output_predictions, indices = deepinsight.analyse.get_model_loss(self.fp_deepinsight, stepsize=10)

    def test05_model_shuffling(self):
        shuffled_losses = deepinsight.analyse.get_shuffled_model_loss(self.fp_deepinsight, axis=1, stepsize=10)
